...
to modify the terminal settings for the slave.  See slave_pty() below.
...
spawn() passes its parameters unchanged to Perls exec(), so look there
...
Sets the tty mode for $object's associated terminal to the given modes. Note
that on many systems the master side of the pty is not a tty, so you
have to modify the slave pty instead, see next item.


=item $object->slave_pty()

Returns a filehandle to the slave part of the pty.  Very useful in modifying
the terminal settings:

  # requires IO::Tty v0.06 or higher!!
  $object->slave_pty->stty(qw(raw -echo));
...
But feel free to experiment with C<$exp-E<gt>slave_pty-E<gt>stty("raw")>.
...
Alternatively, you could be exiting too fast without giving the
spawned program enough time to finish.  Try adding $exp->soft_close()
to terminate the program gracefully or expect() for 'eof'.
...
spawning telnet to port 80, use LWP instead.  You don't use a
...
=head2 How come Expect.pm doesn't run properly under old HP-UX?
...
Also, try IO::Tty v0.05 or newer, which has several fixes and should
work much better on *BSD systems.
...
you expect), so I suggest you set it to 'raw' by yourself:
  ...
  $exp = new Expect;
  $exp->slave_pty->stty(qw(raw));
  $exp->spawn(...);

To disable echo:

  $exp->slave_pty->stty(qw(-echo));
