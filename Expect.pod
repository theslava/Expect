...
The (current) public functions that make up Expect.pm are:
	...
	$object->use_regexps(0, 1, or undef)
	# These are left for backwards compatibility. They are obsolete.
	$object->exp_kill($signal or undef); 
	$object->exp_close();
...
log_group and log_stdout are set.
	Depending on the value of $object->use_regexps() a match may be made
if the pattern matches exactly or if it is a substring found in $object's 
output. It is recommended you set $obects->use_regexps(0) unless you are
specifically looking for a regular expression.
unexpected behavior when you look for thing
...
Sets/unsets whether expect() will take arguments as literals to match or
regular expressions. If set $object will treat all patterns passed to 
expect() as regular expressions. Defaults upon $object creation to the value
of $Expect::Use_Regexps, which by default is 1.
...
value of $Expect::Manual_Stty. See $object->manual_stty().
	...
	Defaults to 1. Newly created objects have a $object->use_regexps()
value of $Expect::Use_Regexps. See $object->use_regexps().
