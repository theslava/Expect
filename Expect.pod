...
=item Expect->exp_init(\*FILEHANDLE)
...
=item Expect->spawn($command, @parameters)
...
=item $object->exp_matchlist();

exp_matchlist() returns a list of matched substrings from the brackets
() inside the regexp that last matched. ($object->exp_matchlist)[0]
thus corresponds to $1, ($object->exp_matchlist)[1] to $2, etc.

=item $object->exp_error();
...
=item $object->log_file("filename" | $filehandle | undef)
