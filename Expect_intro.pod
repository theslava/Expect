...
This document is obsolete and left in for hysterical raisins.  Expect.pod
now is the officially maintained document.  Expect_FAQ.pod should be
of value too.
...
doesn't seem to have a wide spread yet.  You may want to check the 
modules homepage at http://sourceforge.net/projects/expectperl/
and submit any questions or bugreports there.
...
complete clean rewrite in the near future (cannot promise any date)
that may break some backward-compatibility for cleaner design.  For
example, I would like to break it up somewhat, probably move the
interconnect-stuff to a separate module to slim down the core
functionality.  I still want to keep it as close to Tcl/Expect as
possible to make switching real easy.
